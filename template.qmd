...
```{r}
seeds_df <- read_tsv(here::here('data','seeds_dataset.txt')) # :: lazy load pulls in a function from the here package
...
Yes, the min for c, w_k, l_g is -999.000
...
```{r}
pivot_longer <- seeds_df |> 
  pivot_longer(cols = -variety)
  ggplot(pivot_longer, aes(x = value)) + 
    geom_histogram() + 
...
```{r}
scatter1 <- ggplot(data = seeds_df, aes(x = a, y = asym, color = c, shape = variety)) + 
  geom_point() +
  theme_bw()
scatter1
```
...
```{r}
scatter2 <- ggplot(data = seeds_df, aes(x = l_k, y = w_k, color = c, shape = variety)) + 
  geom_point() +
  theme_bw()
scatter2
```
...
There seems to be a positive relationship between kernel width and kernel groove length
...
seeds_scale <- seeds_complete |> 
 select(-variety) |> # the -varitey drops this column from the data frame 
  scale()
...
#| fig-cap: Based on the knee graph, we can determine that we would want three cluster groups to initiate a k-means clusters becasue the kink in the graph is most pronoounced at the clusters

fviz_nbclust(seeds_scale, FUNcluster = kmeans, method = 'wss', k.max = 10) # this is a knee plot :) 
...
plot <- ggplot(data = seeds_cl, aes(x = a, y = asym, color = cluster_no, shape = variety))+ 
  geom_point() + 
  theme_bw()
plot
```
...
It seems like the clusters and varieties are grouping fairly well :) 
However there are some mislabels :o

```{r}

#| tbl-cap: Contiency table for the groupings of kmeans and varietals of kernels 
...
1. Slice out the top 20 GHG emitters 
2. Select only the numeric 
3. Scale ther data 
4. Get the distance of the scaled data 
5. Run hclust 
6. Linkages either single or complete 
7. Make our dendgrograms  
...
wb_ghg_20 <- wb_env |> 
  slice_max(ghg, n=20) # taking the top 20 values from the ghg column
